"""Evaluate one A1-style checkpoint: loss on every corpus, plus samples.
    ...
    python eval/eval.py --run out/cc_english --name cc_english --no-opik

Vendored from assignment 2 so this assignment is self-contained; the
corpora in eval/corpora/ are byte-identical to A2's, so numbers here are
directly comparable to your A2 results.
...
def find_a1():
    """The repository root: model.py and data/vocab.json are vendored there."""
...
def main():
    ...
    ap.add_argument("--a1-dir", default=find_a1(),
                    help="repository root, for model.py and data/vocab.json")
    ap.add_argument("--device", default="cpu",
                    help="the protocol numbers are defined on cpu; cuda matches "
                         "within noise and samples much faster")
    ap.add_argument("--project", default="csci5942-a3", help="Opik project name")
    ...
    with open(os.path.join(args.a1_dir, "data", "vocab.json")) as f:
